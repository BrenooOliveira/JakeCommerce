...
CREATE INDEX idx_cartao_cliente ON cartao(cliente_id);
...
-- Tabela de Pedido (Depende de cliente, endereco, pagamento)
CREATE TABLE pedido (
    id BIGSERIAL PRIMARY KEY,
    data_criacao TIMESTAMP NOT NULL,
    status VARCHAR(50) NOT NULL,
    ...
    CONSTRAINT fk_pedido_pagamento FOREIGN KEY (pagamento_id) REFERENCES pagamento(id) ON DELETE SET NULL
);
...
CREATE INDEX idx_pagamento_cartao_pagamento ON pagamento_cartao(pagamento_id);
...
CREATE INDEX idx_pagamento_cupom_pagamento ON pagamento_cupom(pagamento_id);
